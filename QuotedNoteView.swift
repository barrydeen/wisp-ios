...
struct QuotedNoteView: View {
    ...
    /// Author pubkey carried by the `nevent1…` this quote came from, when it had
    /// one. Only used to attribute a NIP-09 deletion request: a kind-5 counts
    /// solely from the quoted note's own author, and when the note itself can't
    /// be fetched this hint is the only way to know who that is.
